			     ...
			     unsigned long len, void *data)
{
	...
	if (copy_from_user(&cmd_head, buff, CMD_HEAD_LENGTH)) {
		kfree(&cmd_head);
		return -EFAULT;
	}
	...
	if (1 == cmd_head.wr) {
		...
		if (copy_from_user
		    (&cmd_head.data[ADDR_MAX_LENGTH], &buff[CMD_HEAD_LENGTH],
		     cmd_head.data_len)) {
			kfree(&cmd_head);
			return -EFAULT;
		}
		...
		disable_irq(gt_client->irq);
	} else if (9 == cmd_head.wr) {
		enable_irq(gt_client->irq);
	}
